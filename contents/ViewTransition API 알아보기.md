...
그나마 animate 메서드를 활용하면 자바스크립트를 활용해서 동적인 값을 상황에 맞게 결정해서 선언적으로 애니메이션 효과를 줄 수 있어서 활용도가 훨씬 좋아졌지만 여전히 DOM요소가 완전히 바뀌었을 때 두 DOM요소 사이간의 보간(Interpolation)과 전환(Transition)을 같이 처리할 수 있는 방법은 제공되지 않았다. 이런 부족함을 ViewTransition API를 통해 해소할 수 있게 되었다.
...
![화면 전환일 일어나는 모습](/images/example2.gif)
...
ViewTransition API의 멋진 점은 위와 같은 식으로 이전 상태에서 이후 상태로 DOM이 변경될 때 이전 상태의 특정 요소가 이후 상태의 특정 요소로 위치와 크기를 추적하면서 트랜지션을 만들 수 있다는 것이다. 그러기 위해서는 이전 상태의 요소와 이후 상태의 요소를 렌더트리에서 **유일한 key**로 연결해주면 된다. 그리고 그 유일한 key는 CSS의 `view-transition-name` 속성을 활용하면 된다. CSS에서 Detail화면의 title text 요소에 다음과 같이 지정해줄 수 있다.
...
이렇게 해서 Detail 페이지에 있는 유일한 title을 설정해 줄 수 있다. 그런데 List 페이지에 있는 title은 어떻게 지정해 줄 수 있을까? 만약 css를 통해서 `view-transition-name`를 지정한다면 List에 있는 모든 아이템들의 title에 적용되어 이전 상태(List)에서 유일하지 않게 된다. 이런 문제를 해결하기 위해서 자바스크립트를 활용해서 스냅샷을 생성하기 직전에 정의해주면 된다.
...
이렇게 하면 클릭하는 순간에 .list-title을 클래스로 갖는 DOM요소에 직접 style을 적용해줄 수 있다. 그리고 그 이후에 `startViewTransition`메서드를 호출하면 그 시점 렌더트리에 `view-transition-name`이 지정된 요소들의 스냅샷을 만든다. 그리고 `updateCallbackDone`의 상태가 `fulfilled`되면 변경된 렌더트리에서 마찬가지로 `view-transition-name`이 지정된 요소들의 스냅샷을 만든다. 그리고 다음과 같은 의사요소 트리를 생성한다.
